...
(function () {
  ...
  function showSummary() {
    ...
    emailCard.appendChild(createEl('p', { style: { margin: '0 0 10px', color: '#6b7280', fontSize: '14px' } }, [
      'After you submit this intake, email any files (logo, photos, brand colors, inspiration screenshots).'
    ]));

    const mailtoUrl = 'mailto:info@dotcom.website?subject=Website%20Files';
    const gmailComposeUrl = 'https://mail.google.com/mail/?view=cm&fs=1&to=info@dotcom.website&su=Website%20Files';
    const outlookComposeUrl = 'https://outlook.office.com/mail/deeplink/compose?to=info@dotcom.website&subject=Website%20Files';

    emailCard.appendChild(createEl('a', { class: 'dc-btn dc-btn-secondary', href: mailtoUrl }, ['Email Files']));

    const copyBtn = createEl('button', { type: 'button', class: 'dc-btn dc-btn-secondary', id: 'copyEmailBtn', style: { marginLeft: '10px' } }, ['Copy Email Address']);
    emailCard.appendChild(copyBtn);
    emailCard.appendChild(createEl('span', { id: 'copyStatus', style: { marginLeft: '10px' } }, []));

    copyBtn.addEventListener('click', async () => {
      const email = 'info@dotcom.website';
    ...
    emailCard.appendChild(createEl('p', { style: { margin: '10px 0 0', color: '#6b7280', fontSize: '14px' } }, [
      'Or email: ',
      createEl('a', { href: 'mailto:info@dotcom.website', style: { color: 'inherit', textDecoration: 'underline' } }, ['info@dotcom.website'])
    ]));

    emailCard.appendChild(createEl('div', { style: { marginTop: '10px' } }, [
      createEl('a', { class: 'dc-btn dc-btn-secondary', href: gmailComposeUrl, target: '_blank', rel: 'noopener' }, ['Compose in Gmail']),
      createEl('a', { class: 'dc-btn dc-btn-secondary', href: outlookComposeUrl, target: '_blank', rel: 'noopener', style: { marginLeft: '10px' } }, ['Compose in Outlook'])
    ]));
