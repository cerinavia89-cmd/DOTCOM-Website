...
lines.push('');
  ...
  function showSummary() {
    ...
    const form = createEl('form', { class: 'form', method: 'POST', action: 'https://formspree.io/f/xrepeazd', enctype: 'multipart/form-data' }, []);
    ...
    form.appendChild(createEl('input', { type: 'hidden', name: '_next', value: 'thank-you.html' }));
...
// Ensure Formspree submissions redirect to our local thank-you page (not Formspree's default).
// We submit via fetch so redirects work consistently even when Formspree ignores relative _next URLs.
const statusMsg = createEl('p', { style: { marginTop: '10px', color: 'rgba(186,0,0,.85)', fontSize: '14px', display: 'none' } }, ['']);
